...
tests: libwayne_all
	set -x; for x in ebm covar stats hashtest htree-test bintree-test CI graph-sanity; do (cd tests; $(MAKE) $$x; mv $$x ../bin; [ -f $$x.in ] && cat $$x.in | ../bin/$$x $$x.in | if [ -f $$x.out ]; then cmp - $$x.out; else wc; fi); done
